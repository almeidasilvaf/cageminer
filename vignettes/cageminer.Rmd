...
Over the past years, RNA-seq data for several species have accumulated in public
repositories. Additionally, genome-wide association studies (GWAS) have 
...
associated with the phenotype. The goal of `cageminer` is to integrate 
...
3. Find candidates from step 2 that are correlated the condition of interest.

```{r workflow, echo=FALSE, fig.cap="Workflow of the cageminer algorithm."}
knitr::include_graphics("https://raw.githubusercontent.com/almeidasilvaf/cageminer/master/inst/figures/workflow.png")
```
...
After the step 2, we got to `r length(candidates2$candidates)` candidates.
...
Finally, we got to `r length(unique(candidates3$gene))` high-confidence 
